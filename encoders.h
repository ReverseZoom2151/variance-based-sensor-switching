...
volatile long count_e0 = 0; // used by encoder to count the rotation.
volatile byte state_e0 = 0; // used to store the prior and current state.
volatile long count_e1 = 0; // used by encoder to count the rotation.
volatile byte state_e1 = 0; // used to store the prior and current state.

// This ISR handles just Encoder 0
// ISR to read the Encoder 0 Channel A and B pins
...
void setupEncoder0() {
    ...
    EICRB = EICRB | ( 1 << ISC60 );  // using header file names, push 1 to bit ISC60
...
void setupEncoder1() {
    ...
    PCIFR = PCIFR | (1 << PCIF0);  // Clear its interrupt flag by writing a 1.
